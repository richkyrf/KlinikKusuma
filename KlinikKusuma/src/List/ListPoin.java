...
public class ListPoin extends javax.swing.JFrame {
    ...
    public void load() {
        ...
        jcomCari1.setQuery("SELECT `IdPasien` as 'ID', `Tanggal`, `NamaPasien` as 'Pasien', `Total Belanja`, `Pakai Poin`, `Setelah Potong`, `Dapat Poin` FROM (SELECT a.`NoBIlling` as 'No Trans', d.`IdPasien`, DATE_FORMAT(a.`Tanggal`,'%d-%m-%Y') as 'Tanggal', `NamaPasien`, SUM(e.`Jumlah`*e.`Harga`)+SUM(f.`Jumlah`*f.`Harga`) as 'Total Belanja', `Poin` as 'Pakai Poin', (SUM(e.`Jumlah`*e.`Harga`)+SUM(f.`Jumlah`*f.`Harga`) - (`Poin` * 5000)) as 'Setelah Potong', FLOOR((SUM(e.`Jumlah`*e.`Harga`)+SUM(f.`Jumlah`*f.`Harga`) - (`Poin` * 5000)) / 50000) as 'Dapat Poin' FROM `tbbilling`a JOIN `tbperawatan`b ON a.`NoInvoice`=b.`NoInvoice` JOIN `tbantrian`c ON b.`NoAntrian`=c.`NoAntrian` AND b.`Tanggal`=c.`Tanggal` JOIN `tbmpasien`d ON c.`IdPasien`=d.`IdPasien` JOIN `tbbillingobat`e ON a.`NoBilling`=e.`NoBilling` JOIN `tbbillingtindakan`f ON a.`NoBilling`=f.`NoBilling` WHERE 1 AND d.`IdPasien` = '" + ID + "' GROUP BY a.`NoBilling` UNION ALL SELECT a.`NoTransaksi` as 'No Trans', b.`IdPasien`, DATE_FORMAT(b.`Tanggal`,'%d-%m-%Y') as 'Tanggal', `NamaPasien`, SUM(a.`Jumlah`*a.`Harga`) as 'Total Belanja', `Poin` as 'Pakai Poin', (SUM(a.`Jumlah`*a.`Harga`) - (`Poin` * 5000)) as 'Setelah Potong', FLOOR((SUM(a.`Jumlah`*a.`Harga`) - (`Poin` * 5000)) / 50000) as 'Dapat Poin' FROM `tbpenjualandetail`a JOIN `tbpenjualan`b ON a.`NoTransaksi`=b.`NoTransaksi` JOIN `tbmpasien`c ON b.`IdPasien`=c.`IdPasien` WHERE 1 AND b.`IdPasien` = '" + ID + "' GROUP BY a.`NoTransaksi`) t1 ");
