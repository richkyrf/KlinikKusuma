...
public class ListPoin extends javax.swing.JFrame {
    ...
    public void load() {
        ...
        jcomCari1.setQuery("SELECT `ID`, `Tanggal`, `Pasien`, SUM(`Total Belanja`) as 'Total Belanja', SUM(`Total Diskon`) as 'Total Diskon', SUM(`Pakai Poin Tindakan`)+SUM(`Pakai Poin Obat`) as 'Pakai Poin', SUM(`Setelah Potong`) as 'Setelah Potong', IF(SUM(`Pakai Poin Tindakan`)+SUM(`Pakai Poin Obat`) > 0,0,FLOOR((SUM(`Total Belanja`)-SUM(`Total Diskon`)) / 50000)) as 'Dapat Poin' FROM (SELECT a.`IdBilling` as 'ID', a.`Tanggal` as 'Tanggal', a.`NoBilling` as 'No. Billing', `NoTransaksi` as 'No. Transaksi', `NoAntrian` as 'No. Antrian', `NamaPasien` as 'Pasien',SUM((IFNULL(e.`Jumlah`,0)*IFNULL(e.`Harga`,0))) as 'Total Belanja', ROUND(SUM(IF(IFNULL(`DiskonTindakan`,0)>100,IFNULL(`DiskonTindakan`,0),(IFNULL(e.`Jumlah`,0)*IFNULL(e.`Harga`,0))*IFNULL(`DiskonTindakan`,0)/100)),0) as 'Total Diskon' , IFNULL(`PoinTindakan`,0) as 'Pakai Poin Tindakan', 0 as 'Pakai Poin Obat', ROUND(SUM((IFNULL(e.`Jumlah`,0)*IFNULL(e.`Harga`,0))-IF(IFNULL(`DiskonTindakan`,0)>100,IFNULL(`DiskonTindakan`,0),(IFNULL(e.`Jumlah`,0)*IFNULL(e.`Harga`,0))*IFNULL(`DiskonTindakan`,0)/100)) - (IFNULL(`PoinTindakan`,0)*1000),0) as 'Setelah Potong' FROM `tbbilling`a JOIN `tbantrian`b ON a.`IdAntrian` = b.`IdAntrian` JOIN `tbmpasien`c ON b.`IdPasien`=c.`IdPasien` LEFT JOIN `tbbillingtindakan`e ON a.`NoBilling`=e.`NoBilling` WHERE 1 AND b.`IdPasien` = '" + ID + "' GROUP BY c.`IdPasien`, a.`IdBilling`  UNION ALL SELECT a.`IdBilling` as 'ID', a.`Tanggal` as 'Tanggal', a.`NoBilling` as 'No. Billing', `NoTransaksi` as 'No. Transaksi', `NoAntrian` as 'No. Antrian', `NamaPasien` as 'Pasien',(SUM(IFNULL(d.`Jumlah`,0)*IFNULL(d.`Harga`,0))) as 'Total Belanja', ROUND(IF(IFNULL(`DiskonObat`,0)>100,IFNULL(`DiskonObat`,0),(SUM(IFNULL(d.`Jumlah`,0)*IFNULL(d.`Harga`,0)))*IFNULL(`DiskonObat`,0)/100),0) as 'Total Diskon' , 0 as 'Pakai Poin Tindakan', IFNULL(`PoinObat`,0) as 'Pakai Poin Obat', ROUND((SUM(IFNULL(d.`Jumlah`,0)*IFNULL(d.`Harga`,0)))-IF(IFNULL(`DiskonObat`,0)>100,IFNULL(`DiskonObat`,0),(SUM(IFNULL(d.`Jumlah`,0)*IFNULL(d.`Harga`,0)))*IFNULL(`DiskonObat`,0)/100) - (IFNULL(`PoinObat`,0)*1000),0) as 'Setelah Potong' FROM `tbbilling`a JOIN `tbantrian`b ON a.`IdAntrian` = b.`IdAntrian` JOIN `tbmpasien`c ON b.`IdPasien`=c.`IdPasien` LEFT JOIN `tbbillingobat`d ON a.`NoBilling`=d.`NoBilling` WHERE 1 AND b.`IdPasien` = '" + ID + "' GROUP BY c.`IdPasien`, a.`IdBilling` ) t1 GROUP BY `No. Billing` ");
        jcomCari1.setOrder(" ORDER BY `No. Billing` DESC");
        jcomCari1.tampilkan();
        jcomCari1.jtablef.setrender(new int[]{0, 1, 3, 4, 5, 6, 7}, new String[]{"Center", "Date", "Number", "Number", "Center", "Number", "CenteredNumber"});
        setPoin();
    }
    ...
    void setPoin() {
        ...
        dRunSelctOne.setQuery("SELECT `IdPasien`, IFNULL(SUM(`Poin`),0) as 'Poin' FROM (    SELECT `IdPasien`, 0 as 'Jumlah Belanja', 0 as 'Poin' FROM `tbmpasien` WHERE 1 AND `IdPasien` = '" + ID + "'   	UNION ALL     SELECT d.`IdPasien`, (SUM(IFNULL(e.`Jumlah`,0)*IFNULL(e.`Harga`,0))+SUM((IFNULL(f.`Jumlah`,0)*IFNULL(f.`Harga`,0)) - IFNULL(f.`DiskonTindakan`,0))) as 'Total Belanja', FLOOR((SUM(IFNULL(e.`Jumlah`,0)*IFNULL(e.`Harga`,0))+SUM(IFNULL(f.`Jumlah`,0)*IFNULL(f.`Harga`,0)) - IFNULL(f.`DiskonTindakan`,0)) / 50000) as 'Poin' FROM `tbbilling`a LEFT JOIN `tbperawatan`b ON a.`NoTransaksi`=b.`NoTransaksi` JOIN `tbantrian`c ON IF(b.`IdAntrian` IS NULL, a.`IdAntrian`, b.`IdAntrian`)=c.`IdAntrian` JOIN `tbmpasien`d ON c.`IdPasien`=d.`IdPasien` LEFT JOIN `tbbillingobat`e ON a.`NoBilling`=e.`NoBilling` LEFT JOIN `tbbillingtindakan`f ON a.`NoBilling`=f.`NoBilling` WHERE 1 ANd `PoinTindakan` = 0 AND `PoinObat` = 0 AND d.`IdPasien` = '" + ID + "' GROUP BY d.`IdPasien`, a.`NoBilling`         UNION ALL         SELECT d.`IdPasien`, (a.`PoinTindakan`+a.`PoinObat`)*1000*-1 as 'Total Belanja', (`PoinTindakan`+`PoinObat`)*-1 FROM `tbbilling`a LEFT JOIN `tbperawatan`b ON a.`NoTransaksi`=b.`NoTransaksi` JOIN `tbantrian`c ON IF(b.`IdAntrian` IS NULL, a.`IdAntrian`, b.`IdAntrian`)=c.`IdAntrian` JOIN `tbmpasien`d ON c.`IdPasien`=d.`IdPasien` LEFT JOIN `tbbillingobat`e ON a.`NoBilling`=e.`NoBilling` LEFT JOIN `tbbillingtindakan`f ON a.`NoBilling`=f.`NoBilling` WHERE 1 AND (a.`PoinTindakan` > 0 OR a.`PoinObat` > 0) AND d.`IdPasien` = '" + ID + "' GROUP BY d.`IdPasien`, a.`NoBilling`	) t1 WHERE 1 GROUP BY `IdPasien`");
